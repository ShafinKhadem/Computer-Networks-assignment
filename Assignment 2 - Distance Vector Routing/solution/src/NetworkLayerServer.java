...
public class NetworkLayerServer {
    ...
    public static void main(String[] args) {
        ...
        while(true) {
            try {
                ...
                clientCount++;
                activeClients.add(endDevice.getIpAddress());
    ...
    public static void blockStateChanger() {
        RouterStateChanger.islocked = true;
    }

    public static void unblockStateChanger() {
    ...
    public static synchronized void DVR(int startingRouterId) {
        ...
        */
        ...
        blockStateChanger();
        ArrayList<Router> order = new ArrayList<>(routers);
        Router startingRouter = routerMap.get(startingRouterId);
        order.remove(startingRouter);
        order.add(0, startingRouter);
        ArrayList<ArrayList<Router>> activeNeighborsOrder = new ArrayList<>();
        for (Router r :
                order) {
            ArrayList<Router> activeNeighbors = new ArrayList<>();
            for (Integer n :
                    r.getNeighborRouterIDs()) {
                Router t = routerMap.get(n);
                if (t.getState()) activeNeighbors.add(t);
            }
            activeNeighborsOrder.add(activeNeighbors);
        }
...
//            System.out.println("not converged");
            ...
            for (int i = 0; i < order.size(); i++) {
                for (Router t :
                        activeNeighborsOrder.get(i)) {
                    convergence |= t.getState() && order.get(i).getState() && t.sfupdateRoutingTable(order.get(i));
                }
            }
        }
        unblockStateChanger();
    ...
    public static synchronized void simpleDVR(int startingRouterId) {
        ...
        while (convergence) {
            ...
            for (int i = 0; i < order.size(); i++) {
                for (Router t :
                        activeNeighborsOrder.get(i)) {
                    convergence |= t.getState() && order.get(i).getState() && t.updateRoutingTable(order.get(i));
                }
            }
        }
        unblockStateChanger();
